    ...
    <View style={{ flex: 1, backgroundColor: '#E0EED6', }}>
      ...
      <CustomHeader title={`Add ${title}`} navigation={navigation}/>
        <View style={{ ...styles.body }}>
          ...
          <TouchableOpacity onPress={()=>setDatePickerVisibility(true)} style={{flexDirection:'row',justifyContent:'space-between',backgroundColor:colors.white,paddingHorizontal:10,alignSelf:'flex-end',paddingVertical:8,borderRadius:10,width:'49%'}}>
            <Text>{expiryDate||'Date'}</Text>
            <Icon name='calendar' size={20} color={colors.primary}/>
          </TouchableOpacity>
          </View>
          <View style={{ marginTop: 14 }}>
            <TouchableOpacity disabled={selectItemId==0||!weight} onPress={onAdd} style={{...styles.button,backgroundColor:(selectItemId==0||!weight)?colors.secondary:colors.primary}}>
